...
def search_privacy_law(query: str) -> str:
    """Search privacy and data protection law knowledge base for GDPR, CCPA, and data breach regulations.

    Args:
        query: Natural language query about privacy or data protection.
    """
    knowledge = [
        (
            ["data", "privacy", "gdpr", "ccpa", "consent", "user", "leak"],
            "CCPA: fines up to $7,500 per intentional violation. GDPR: up to 4% of global "
            "revenue or EUR 20M. FTC Act Section 5 for unfair/deceptive practices. "
            "Class action exposure under state privacy laws ($100-$750 per consumer). "
    ...
    messages = [
        SystemMessage(
            content=(
                ...
                'needs_privacy = true → question involves data privacy, GDPR, CCPA, user data leak'
            )
        ),
        HumanMessage(content=state["question"]),
    ]
...
async def call_privacy_specialist(state: LegalState) -> dict:
    ...
    privacy_prompt = (
        "You are a specialist privacy and data protection attorney with expertise in GDPR, CCPA, "
...
async def main():
    ...
    # IPython Display drawing Mermaid graph (Bước 3)
    try:
        from IPython.display import Image, display
        display(Image(graph.get_graph().draw_mermaid_png()))
        print("  [Mermaid Graph] Rendered graph via IPython display successfully!")
        ...
        try:
            ...
            with open(os.path.join(os.path.dirname(__file__), "graph.png"), "wb") as f:
                f.write(png_data)
            print("  [Mermaid Graph] Saved graph diagram to stages/stage_4_milti_agent/graph.png")
        except Exception as file_err:
            print(f"  [Mermaid Graph] Could not draw or save graph: {file_err}")
